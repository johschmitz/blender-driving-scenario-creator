...
def test_longitudinal_distance_uses_library_action_schema():
    ...
    assert distance.attrib['displacement'] == 'any'
    assert distance.attrib['freespace'] == 'true'
    constraints = action.find('.//DynamicConstraints')
    assert constraints.attrib == {
        'maxSpeed': '30.0',
        'maxDeceleration': '4.0',
        'maxAcceleration': '2.0',
    }
...
def test_longitudinal_distance_can_target_time_gap_instead_of_distance():
    root = ET.fromstring(scenario_node_xml.serialize_storyboard([
        node('TimeGap', 'longitudinal_distance', distance_target_type='time_gap', time_gap=1.5),
    ]))
    distance = root.find('./Storyboard/Story/Act/ManeuverGroup/Maneuver/Event/Action/PrivateAction/LongitudinalAction/LongitudinalDistanceAction')
    assert 'distance' not in distance.attrib
    assert distance.attrib['timeGap'] == '1.5'
